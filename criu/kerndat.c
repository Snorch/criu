...
static int __mount_and_stat_detached_binfmt_misc(struct stat *st)
{
	int exit_code = 0, mnt_fd;
...
static int __has_binfmt_misc_sandboxing(void *arg)
{
	...
	ret = __mount_and_stat_detached_binfmt_misc(&st);
	if (ret < 0) {
		return 1;
	} else if (ret == 1) {
		has_arg->has = false;
...
static int kerndat_has_binfmt_misc_sandboxing(void)
{
	...
	if (call_in_child_process(__has_binfmt_misc_sandboxing, (void *)&arg))
		return -1;
