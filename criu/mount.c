 ...
 * (visible) and which is beneath (hidden). It will break mount restore order
 * in can_mount_now and break visibility assumptions in open_mountpoint.
 *
 * After kernel v4.11 it would be imposible to have such mounts at all, so just
 * dont support them.
 */
...
shared:
	...
	/* Mount non-remaped sibling-overmounts only after mounts under them */
	if (mi->parent) {
		struct mount_info *s;
	...
	/*
	 * Monut only after all children of share, which shouldn't
