 ...
 */
static struct mount_info *mnt_get_external_bind(struct mount_info *mi)
{
	return mnt_bind_pick(mi, __mnt_is_external_bind);
}

static bool mnt_is_external_bind(struct mount_info *mi)
{
...
{
	if (bind->external && bind->master_id == mi->master_id && is_sub_path(mi->root, bind->root))
		return true;

	return false;
}

static bool can_receive_master_from_external(struct mount_info *mi)
{
...
static bool has_mounted_external_bind(struct mount_info *mi)
{
...
static bool can_mount_now(struct mount_info *mi)
{
	...
	if ((ext = mnt_get_external_bind(mi)) && !has_mounted_external_bind(mi)) {
...
static int do_mount_one(struct mount_info *mi)
{
	...
	if (rst_mnt_is_root(mi)) {
		...
		mi->mounted = true;
		ret = 0;
	} else if (!mi->bind && !mi->need_plugin && (!mi->external || mnt_is_dev_external(mi)))
		ret = do_new_mount(mi);
	else
